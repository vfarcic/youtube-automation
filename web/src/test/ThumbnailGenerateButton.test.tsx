...
describe('ThumbnailGenerateButton', () => {
  ...
  it('renders 2 tiles per provider (one per B&W style)', async () => {
    // Default handler returns 2 styles per provider.
    renderButton({ tagline: 'Test Tagline' });
    await userEvent.click(screen.getByRole('button', { name: 'Generate Thumbnails' }));
    await waitFor(() => {
      expect(screen.getAllByRole('button', { name: 'Use This' })).toHaveLength(4);
  ...
  // Rapid double-click on Generate must not fire two generate requests: the
  // button disables while generateMutation is in flight.
  it('rapid double-click on Generate does not duplicate generate calls', async () => {
    let generateCount = 0;
    server.use(
      http.post('/api/thumbnails/generate', async () => {
        generateCount++;
        // Delay so the request is visibly in flight when the second click lands.
    ...
    // Wait for the chain to finish so the count is stable.
    await waitFor(() => {
      expect(screen.getByText('with illustration')).toBeInTheDocument();
    });
